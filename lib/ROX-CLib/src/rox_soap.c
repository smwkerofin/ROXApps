...
#define VERSION "1.1.9"         /* Version of ROX-Filer to contact */
...
void rox_soap_init(void)
{
  ...
  id=g_strdup_printf("_ROX_FILER_%d_%s_%s",
				(int) geteuid(), VERSION, hostn);
...
ROXSOAP *rox_soap_connect_to_filer(void)
{
  ...
  filer->existing_ipc_window=get_existing_ipc_window();
		      ...
		      gpointer data)
{
  ...
  /* TODO: If we got a reply, get it here */
  if(sdata->callback)
    sdata->callback(sdata->filer, TRUE, NULL, sdata->data);
				...
				rox_soap_callback callback, gpointer udata)
{
  ...
  dprintf(3, "child is %d, monitor %d", sdata->child, fch[1]);
  close(fch[0]);
  close(tch[1]);
  sdata->fd=fch[1];
